...
public class LocalUDPDataReciever {
        ...
        this.thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    System.out.println(LocalUDPDataReciever.TAG + "本地UDP端口侦听中...");

                    //开始侦听
                    LocalUDPDataReciever.this.udpListeningImpl();
                } catch (Exception eee) {

                    System.out.println(LocalUDPDataReciever.TAG + "本地UDP监听停止了(socket被关闭了?)," + eee.getMessage());
                }
            }
        });
        this.thread.start();
    ...
    private void udpListeningImpl() throws Exception {
        while (true) {
            ...
            DatagramSocket localUDPSocket = EchoClient.getDatagramSocket();
            ...
            System.out.println(LocalUDPDataReciever.TAG + "【NOTE】>>>>>> 收到服务端的消息：" + pFromServer);
        }
    }
}